...
from src.documents.service import document_connection_manager, \
    create_document_in_db, get_documents_from_db, \
        update_document_name_in_db, get_document_from_db, delete_document_from_db
...
def delete_documents(room_uuid: str, document_uuid: str, request: Request):
    document = delete_document_from_db(room_uuid, document_uuid)
    return JSONResponse(
        status_code=200,
        content=document
    )
...
def update_documents(room_uuid: str, document_uuid: str, document_dto: document_update_dto, request: Request):
    document = update_document_name_in_db(room_uuid, document_uuid, document_dto.name)
    return JSONResponse(
        status_code=200,
        content=document
    )
